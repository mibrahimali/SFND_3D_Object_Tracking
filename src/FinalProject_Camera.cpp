...
int main(int argc, const char *argv[])
{
    ...
    fstream outputFile;
    string resultDir = dataPath + "analysis_results/";
    string objDetectionResultPath = resultDir + "object_Detections/";
    string lidarImagesPath = resultDir+"Cropped_Lidar/";
    string finalTTCImagesPath = resultDir + "final_ttc/";
    string filename = resultDir + "analysis.csv"; // create a name for the file output
    outputFile.open(filename, ios::out);
    

    // define list of possible Detector / Descriptor for testing
    vector<string> detectors = {"HARRIS",
                                "SHITOMASI",
                                "FAST",
                                "ORB",
                                "BRISK",
                                "SIFT"};
    vector<string> descriptors = {"BRISK","ORB","BRIEF", "SIFT"};
    ...
    for (auto detector : detectors)
    {
        for (auto descriptor : descriptors)
        {
            ...
            {
                continue; // workaround of out of memory issue between SIFT detector and ORB descriptor
            }
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
            {
                ...
                bVis = true; // this will write images instead of show them
                string save_path = objDetectionResultPath + imgNumber.str() + imgFileType;
                detectObjects((dataBuffer.end() - 1)->cameraImg, (dataBuffer.end() - 1)->boundingBoxes, confThreshold, nmsThreshold,
                              yoloBasePath, yoloClassesFile, yoloModelConfiguration, yoloModelWeights, bVis, save_path);
                ...
                if (bVis)
                {
                    save_path = lidarImagesPath + imgNumber.str() + imgFileType;
                    show3DObjects((dataBuffer.end() - 1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(2000, 2000),save_path);
                }
                bVis = false;
                ...
                descKeypoints((dataBuffer.end() - 1)->keypoints, imgGray, descriptors, descriptorType);
                    ...
                    for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                    {
                        ...
                        if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                        {
                            ...
                            if (bVis)
                            {
                                ...
                                save_path = finalTTCImagesPath +detectorType+"_"+descriptorType+"_"+ imgNumber.str() + imgFileType;
                                cv::imwrite(save_path.c_str(),visImg);
                            ...
                            outputFile << detectorType << ", " << descriptorType << ", " << imgIndex << ", "
                                       << ttcLidar << ", " << ttcCamera << ", " << (ttcLidar - ttcCamera) << "\n";
